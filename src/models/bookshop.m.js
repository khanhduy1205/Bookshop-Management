...
module.exports = {
    ...
    addBookToDB: async (b) => {
        const result = await db.one(
            'INSERT INTO "Books"("bookID", "bookname", "category", "author", "quantity", "price") VALUES($1, $2, $3, $4, $5, $6) RETURNING *',
    ...
    byBookName: async (bookname) => {
        const result = await db.one('SELECT * FROM "Books" WHERE bookname=$1', [bookname]);
        return result;
    },
    byBookID: async (bookID) => {
        const result = await db.one('SELECT * FROM "Books" WHERE "bookID"=$1', [bookID]);
        return result;
    },
    editBook: async (book) => {
        const result = await db.none(
            'UPDATE "Books" SET "bookname"=$1, "category"=$2, "author"=$3, "quantity"=$4, "price"=$5 WHERE "bookID"=$6',
    ...
    updateQuantity: async (bookID, quantity) => {
        const rs = await db.none('UPDATE "Books" SET "quantity"=$2 WHERE "bookID"=$1', [bookID, quantity]);
        return rs;
    },
};
